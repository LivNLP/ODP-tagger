...
#sys.path.append(os.path.abspath(os.path.join(os.path.curdir, 'seq_models')))
from bi-lstm import Lstm_model
import numpy as np
    ...
    if str(model_name).lower() == 'lstm':
        ...
        for epoch in range(n_epochs):
            ...
            for sentence, tags in inp_data:
                ...
                target, predicted_sentence_list = utils.extract_predictions_from_model_output(tag_scores, tags, index2tag)
                ...
                sim, total = utils.token_level_accuracy(target, predicted_sentence_list)
                s += sim
                t += total
    ...
    with torch.no_grad():
        with open(os.path.join(file_dir, '{}.out'.format(decoded_file_name)), 'w') as d, open(os.path.join(file_dir, 'baseline_model_results.txt'), 'w') as h:
            ...
            for sentence, tags in test_data:
                ...
                target, predicted_sentence_list = utils.extract_predictions_from_model_output(model_output, tags, index2tag)
            ...
            metrics = utils.metrics_score(T, P, list(tag_map.keys()))
            print(metrics)
            h.write(metrics)
            d.close()
            h.close()
